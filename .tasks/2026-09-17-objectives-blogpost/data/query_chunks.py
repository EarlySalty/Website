...
def window(w):
    return (f"start_time >= TIMESTAMPTZ '{w[1]}' AND start_time < TIMESTAMPTZ '{w[2]}' "
            "AND game_mode = 'Normal' AND match_mode = 'Ranked'")


def qa_sql(w):
    return (
        "SELECT epoch(date_trunc('day', start_time))::BIGINT AS day_epoch, " + TIERS_ROW + " AS tier, "
        ...
        "FROM match_player WHERE " + window(w) + " GROUP BY day_epoch, tier ORDER BY day_epoch, tier"
    )
...
def qb_sql(w):
    return (
        ...
        "  FROM match_player WHERE " + window(w) + " AND match_outcome = 'TeamWin' GROUP BY match_id"
        "),"
        "mb AS ("
...
def qd_sql(w):
    WT = window(w) + " AND match_outcome = 'TeamWin'"
    return (
        "WITH raw AS ("
        ...
        "  FROM match_player WHERE " + WT +
        "),"
        "pd AS ("
        "  SELECT match_id, account_id, "
        "    arg_max(team, created_at) AS team, arg_max(average_badge, created_at) AS average_badge, "
        "    arg_max(winning_team, created_at) AS winning_team, arg_max(ts, created_at) AS ts, "
        "    arg_max(nw, created_at) AS nw, arg_max(gt, created_at) AS gt, "
        "    arg_max(mb_t, created_at) AS mb_t, arg_max(mb_c, created_at) AS mb_c, "
        "    arg_max(ob_n, created_at) AS ob_n, arg_max(ob_t, created_at) AS ob_t, "
        ...
        "    coalesce((mb_first_c = wt)::INT, 0) + coalesce((shr_first_team = wt)::INT, 0) "
        ...
        "count(*) FILTER (mb_first_t IS NOT NULL AND shr_first_t IS NOT NULL AND urn_first_t IS NOT NULL "
...
SQLS = {"qa": qa_sql, "qb": qb_sql, "qd": qd_sql}
...
if __name__ == "__main__":
    ...
    for name, start, end in WEEKS:
        if only and name != only:
            continue
        fname = f"{kind}_{name}.json"
        t0 = time.time()
        try:
            res = run_query(SQLS[kind]((name, start, end)))
