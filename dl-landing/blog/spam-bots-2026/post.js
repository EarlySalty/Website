...
const { renderBars, renderGroupedBars, renderHBars, renderStackedBars } = createCharts({ prefix: 'bl' });
...
})), { height: 240, valueOnMax: false, ariaLabel: 'Vorfälle je 10.000 Nachrichten je Monat' });
...
}), { height: 240, ariaLabel: 'Verfremdungsformen der Werbetexte je Monat', series: stackFormen });
...
buildTable('marken',
  ['Marke', 'Erstauftritt', 'Vorfälle', 'Kanäle', 'Lebensdauer (Tage)', 'Filter-Fragment gelernt', 'Reaktion bis Fragment (Tage)'],
  MARKEN.map((m) => [
    m.marke,
...
renderHBars(q('[data-chart="banrate"]'), BANRATE.map((m) => ({
  ...
  sub: `${fmt(m.mit_aktion)} von ${fmt(m.vorfaelle)} Vorfällen`,
  value: m.anteil,
  display: fmt1(m.anteil),
})), { unit: ' %', maxValue: 100 });

buildTable('banrate', ['Monat', 'Vorfälle', 'Mit Aktion', 'Anteil'],
...
const fills = {
  ...
  'c5-schwanz-spam': fmt1(NAMENSMUSTER.spam.zufallsschwanz),
  'c5-schwanz-normal': fmt1(NAMENSMUSTER.normal.zufallsschwanz),
  ...
  'c6-aug': fmt1(BANRATE[1].anteil),
  'c6-sep': fmt1(BANRATE[2].anteil),
  'c6-median': fmt1(REAKTIONSZEIT.median),
  'c6-p90': fmt1(REAKTIONSZEIT.p90),
  'c6-spam': fmt(PFADE.spam),
  'c6-scam': fmt(PFADE.scam),
  'c6-guardbans': fmt(PFADE.scam_guard_bans),
  'c6-global': fmt(PFADE.global_ban),
  'c6-banliste-konten': fmt(GLOBAL_BANLISTE.konten),
  'c6-banliste-anwendungen': fmt(GLOBAL_BANLISTE.anwendungen),
  'c6-banliste-kanaele': fmt(GLOBAL_BANLISTE.kanaele),
  'c6-regex-harmlos': fmt(FEHLALARME.regex_harmlos),
  'c6-regex-geprueft': fmt(FEHLALARME.regex_geprueft),
  'c6-safe': fmt(FEHLALARME.safe_patterns),
